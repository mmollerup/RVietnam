...
- `ggplot()`: `ggplot2`´s plot function 
- `aes`: aesthetics
- `geom_XX()`: geoms for creating plots
- `labs()`: labels 

<!-- - `ggpot()`: ggplot's plotting device -->
...
```{r, echo=FALSE, warning=FALSE, dev.args=list(bg="transparent")}
...
```{r, warning=FALSE, message=FALSE, out.width = "75%", fig.align='center', dev.args=list(bg="transparent")}
boys %>% filter(!is.na(reg)) %>%
ggplot(aes(x=reg)) +
